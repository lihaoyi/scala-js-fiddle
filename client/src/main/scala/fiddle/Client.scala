...
object Client{
  ...
  lazy val original = canvas.cloneNode(true)
  def clear() = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    dom.setTimeout((() => main2()): js.Function0[_], 1000)
  }
  def main2() = {
    ...
    val callback = { () =>
      ...
      cache.get(code) match {
        case Some((spam, Some(compiled))) =>
          clear()
          js.eval(compiled)
          js.eval("ScalaJS.modules.Main().main()")
        case Some((spam, None)) =>
          logspam.textContent = spam
        ...
        case None =>
          ...
          if (!requestInFlight){
            ...
            req.open("POST", "http://0.0.0.0:8080")
            req.send(code)
